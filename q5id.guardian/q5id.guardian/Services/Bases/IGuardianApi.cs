...
namespace q5id.guardian.Services.Bases
{
    public interface IGuardianApi
    {
        ...
        Task<AppServiceResponse<Entity<T>>> CreateEntity<T>(string instancesId, [Body] Object body, CancellationToken cancellationToken) where T : BaseEntity;
        
        [Put("/datavaultdata/entitydata/instances/{instancesId}/{primaryId}")]
        Task<AppServiceResponse<Entity<T>>> UpdateEntity<T>(string instancesId, string primaryId, [Body] object body, CancellationToken cancellationToken) where T : BaseEntity;
